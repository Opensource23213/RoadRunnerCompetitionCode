 ...
 */
@Config
@TeleOp(name="BucDays", group="ABC Opmode")
//@Disabled
public class BucDays extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if(gamepad2.dpad_down && gamepad2.right_trigger > .4){
                topose = 2;
                xopen = 0;
                ymove = 1;
                ypress = 1;
                ...
                serPosition = 0;
                position = -11.45;
            }else if(gamepad2.dpad_left && gamepad2.right_trigger > .4){
                topose = 2;
                xopen = 0;
                ymove = 1;
                ypress = 1;
                ...
                serPosition = 0;
                position = -6.44;
            }else if(gamepad2.dpad_up && gamepad2.right_trigger > .4){
                topose = 2;
                xopen = 0;
                ymove = 1;
                ypress = 1;
...
// Field Centric Calculations
            ...
            if (yes == 2){ // while a power modifying button is being used
                ...
                if(ypress == 3){
                    if(gamepad1.dpad_right) {
                        leftFrontPower = (axial + lateral + (yaw + 2.5 * .02)) * power_level;
                        rightFrontPower = (axial - lateral - (yaw + 2.5 * .02)) * power_level;
                        leftBackPower = (axial - lateral + (yaw + 2.5 * .02)) * power_level;
                        rightBackPower = (axial + lateral - (yaw + 2.5 * .02)) * power_level;
                    }
            ...
            if (topose == 1) {
                ...
                double ff = Math.cos(Math.toRadians(position + 22)) * f;
                double power = pid + ff;
                Arm1.setPower(power);
                Arm2.setPower(power);
